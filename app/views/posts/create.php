...
<section class="uk-container uk-margin-large-top" uk-height-viewport="expand: true">
  <ul class="uk-breadcrumb">
    <li><a href="<?= $this->path_for('admin') ?>">Admin </a></li>
    <li><a href="<?= $this->path_for('post') ?>">Todos os post </a></li>
    <li><span>Adicionar Post </span></li>
  </ul>
  ...
  <form class="uk-margin-medium-top" action="<?= $this->path_for('post.create') ?>  " method="POST" enctype="multipart/form-data" id="form" name="formulario">
    <fieldset class="uk-fieldset">

      <div class="uk-margin">
        <p class="title_form">Titulo: </p><input class="uk-input" name="title" type="text" maxlength="300" placeholder="" value="<?= $this->old('title') ?>" />
      </div>
      ...
      <div class="uk-margin">
        <p class="title_form">Texto do post: </p>
        <div class="uk-margin">
          <textarea id="editor" name="conteudo" value="<?= $this->old('conteudo') ?>"></textarea>
        </div>
      </div>
      ...
      <div class="uk-margin">
        <p class="title_form">Descrição: </p><input class="uk-input" name="description" type="text" maxlength="200" placeholder="" value="<?= $this->old('description') ?>" />
      </div>
      ...
      <div class="uk-margin">
        ...
        <select class="uk-select" name="published">
          <?php $oldpublished = $this->old('published'); ?>
          <option value="0" <?= $oldpublished == '0' ? 'selected' : '' ?>>
            Desativado
          </option>
          <option value="1" <?= $oldpublished == '1' ? 'selected' : '' ?>>
            Ativado
          </option>
        </select>
      ...
      <div class="uk-margin" uk-margin>
        <p class="title_form">Imagem do banner: </p>
        <div uk-form-custom><input type="file" id="file-chooser" accept="image/*, video/*" name="banner" /><button class="uk-button uk-button-default" type="button" tabindex="-1">Selecione uma imagem </button></div>
        <div class="w535"><img id="preview-img" class=" banner_img uk-display-block uk-responsive-width" width="400" /></div>
        <p><input class="uk-checkbox" type="checkbox" name="apagar_img" value="1">Deixar sem imagem </p>
      </div>
      ...
      <?php if ($bannerError) : ?>
        <div class="uk-margin-top">
          <?php echo $bannerError ?>
        </div>
      <?php endif; ?>

      <div class="uk-margin" uk-margin>
        <p class="title_form">Imagem da thubmail: </p>
        <div uk-form-custom><input type="file" id="file-chooser_b" type="file" accept="image/*" maxlength="150" name="thumb" /><button class="uk-button uk-button-default uk-block" type="button" tabindex="-1">Selecione a imagem </button></div><img id="preview-img_b" class="banner_img uk-display-block uk-responsive-width" width="400" />
        <p><input class="uk-checkbox" type="checkbox" name="apagar_img2" value="1" />Deixar sem imagem </p>
      </div>

      <?php if ($thumbeError) : ?>
        <div class="uk-margin-top">
          <?php echo $thumbeError ?>
        </div>
      <?php endif; ?>

      <div class="uk-grid">
        <div class="uk-margin"><button type="submit" class="uk-button salvar-f" id="salvar">Salvar </button></div>
        <div><a href="<?= $this->path_for('post') ?>" class="uk-button voltar-f uk-margin-medium-bottom">Voltar </a></div>
      </div>

      <a hidden href="#modal-center" id="acionar-btn" uk-toggle>Open </a>
      ...
      <div id="modal-center" class="" uk-modal>
        <div class="uk-modal-dialog"><button class="uk-modal-close-default" type="button" uk-close></button>
          <div class="uk-modal-header">
            <h2 class="uk-modal-title uk-text-danger">ALERTA </h2>
          </div>
          <div class="uk-modal-body texto_alerta_form">
            <p id="text-mensage"></p>
            <p>Deseja salvar mesmo assim? </p>
          </div>
          <div class="uk-modal-footer uk-text-right"><button class="uk-button voltar-f uk-modal-close">Voltar </button><button class="uk-button salvar-f" type="submit" id="salvar2">Salvar </button></div>
        </div>
      </div>
...
CKEDITOR.replace('editor', {
        ...
        autoGrow_minHeight: 200,  // Altura mínima do editor em pixels.
        autoGrow_maxHeight: 600,  // Altura máxima do editor. 0 significa ilimitado.
        autoGrow_bottomSpace: 50, // Espaço extra após o conteúdo.
        height: 200                // Altura inicial do editor.
    });
    // Quando o documento estiver pronto, inicialize o editor TUI
